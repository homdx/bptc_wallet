...
class PullServer(protocol.Protocol):
    ...
    def connectionMade(self):
        ...
        serialized_events = {}
        for event_id, event in self.factory.network.hashgraph.lookup_table.items():
            serialized_events[event_id] = SerializableDebugEvent(event.data, event.parents, event.height, event.time,
                                                                 str(event.verify_key), event.round)
...
class PullClient(protocol.Protocol):
    ...
    def dataReceived(self, data):
        ...
        for event_id, s_event in s_events.items():
            events[event_id] = Event.create_from_serializable_debug_event(s_event)
